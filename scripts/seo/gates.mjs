...
export async function checkDedupe(candidateText, existingPages, t = THRESHOLDS) {
  ...
  let worst = { url: null, score: -1 };
  for (let i = 0; i < existingPages.length; i++) {
    const s = cosine(cand, vectors[i + 1]);
    if (s > worst.score) worst = { url: existingPages[i].path, score: s };
  }
  const pass = worst.score < t.COSINE_REJECT;
  return {
    pass,
    detail: `max cosine ${worst.score.toFixed(3)} vs ${worst.url} (${providerId}, threshold ${t.COSINE_REJECT}, n=${existingPages.length})`,
  };
}
