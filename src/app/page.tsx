...
export default async function Home() {
  ...
  // SEO: fetch article data for content hub sections
  const [trendingArticles, articleCounts] = await Promise.all([
    prisma.article.findMany({
      where: { published: true },
      orderBy: { publishedAt: "desc" },
      take: 3,
      select: { slug: true, title: true, description: true, wordCount: true, vertical: true, pageType: true },
    }),
  ...
  const totalArticles = articleCounts.reduce((s, r) => s + r._count, 0);
  const countByType = Object.fromEntries(articleCounts.map((r) => [r.pageType, r._count]));
  ...
  const homepageFaq = faqSchema([
    ...
    { question: "How do I prepare for a product manager interview?", answer: "PM Streak offers dedicated interview guides covering behavioral questions, product sense frameworks, estimation techniques, and case studies — all based on patterns from top PM interviews at Google, Meta, Amazon, and Stripe." },
    { question: "What is a north star metric in product management?", answer: "A north star metric is the single metric that best captures the core value your product delivers to customers. For example, Spotify's north star is time spent listening; Airbnb's is nights booked. PM Streak has a full guide on choosing the right north star metric for your product." },
    { question: "How do product managers prioritize features?", answer: "Product managers use prioritization frameworks like RICE (Reach, Impact, Confidence, Effort), ICE scoring, and the Kano model to rank features. PM Streak covers all major prioritization frameworks sourced from Lenny's Podcast episodes with Shreyas Doshi, Gibson Biddle, and other top PMs." },
  ]);
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] text-white">
      ...
      {/* ── KNOWLEDGE BASE: TOPIC CLUSTERS ── */}
      {totalArticles > 0 && (
        <section className="max-w-5xl mx-auto px-4 sm:px-5 py-14 sm:py-16">
          <div className="flex items-end justify-between gap-4 mb-8">
            <div>
              <p className="text-xs font-black uppercase tracking-widest text-[var(--green-primary)] mb-2">Free PM knowledge base</p>
              <h2 className="text-2xl sm:text-3xl font-black leading-tight tracking-tight">
                {totalArticles}+ in-depth PM guides
              </h2>
              <p className="text-sm text-[var(--text-secondary)] mt-2 max-w-md leading-relaxed">
                Expert-reviewed articles on every PM topic — from interview prep to product strategy to metrics.
              </p>
            </div>
            <Link
              href="/learn"
              className="hidden sm:inline-flex items-center gap-1.5 text-sm font-black text-[var(--green-primary)] hover:opacity-80 transition-opacity flex-shrink-0"
            >
              Browse all →
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            {[
              { label: "Interview Prep", count: countByType["interview_guide"] ?? 0, icon: "🎤", href: "/learn?type=interview_guide", desc: "Case studies, behavioral, estimation" },
              { label: "Frameworks", count: countByType["framework"] ?? 0, icon: "🗺️", href: "/learn?type=framework", desc: "RICE, OKRs, CIRCLES, Kano" },
              { label: "Strategy & Growth", count: Math.round((countByType["blog"] ?? 0) * 0.3), icon: "📈", href: "/learn", desc: "North star, roadmaps, GTM" },
              { label: "PM Career", count: Math.round((countByType["blog"] ?? 0) * 0.2), icon: "🚀", href: "/learn", desc: "Salary, progression, hiring" },
            ].map(({ label, count, icon, href, desc }) => (
              <Link
                key={label}
                href={href}
                className="group flex flex-col gap-2 p-4 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl hover:border-[var(--green-primary)]/40 hover:bg-[var(--bg-secondary)] transition-all"
              >
                <span className="text-2xl">{icon}</span>
                <div>
                  <div className="font-black text-sm leading-tight mb-0.5">{label}</div>
                  <div className="text-[10px] text-[var(--green-primary)] font-bold">{count > 0 ? `${count} guides` : "guides"}</div>
                  <div className="text-[10px] text-[var(--text-secondary)] mt-1 leading-tight hidden sm:block">{desc}</div>
                </div>
              </Link>
            ))}
          </div>
          ...
          <Link
            href="/learn"
            className="sm:hidden inline-flex items-center gap-1.5 text-sm font-black text-[var(--green-primary)] hover:opacity-80 transition-opacity"
          >
      ...
      {trendingArticles.length > 0 && (
        <section className="border-t border-[var(--border-color)] bg-[var(--bg-secondary)]/30">
          <div className="max-w-5xl mx-auto px-4 sm:px-5 py-12 sm:py-14">
            <div className="flex items-center justify-between gap-4 mb-6">
              <p className="text-xs font-black uppercase tracking-widest text-[var(--text-secondary)]">Recently published</p>
              <Link href="/learn" className="text-xs font-black text-[var(--green-primary)] hover:opacity-80 transition-opacity">
                View all →
              </Link>
            </div>
            <div className="grid sm:grid-cols-3 gap-3 sm:gap-4">
              {trendingArticles.map((a) => {
                const readTime = Math.ceil(a.wordCount / 200);
                return (
                  <Link
                    key={a.slug}
                    href={`/learn/${a.vertical}/${a.slug}`}
                    className="group flex flex-col gap-2 p-5 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl hover:border-white/20 hover:bg-[var(--bg-secondary)] transition-all"
                  >
                    <span className="text-[10px] font-black uppercase tracking-widest text-[var(--green-primary)]">
                      {a.pageType === "interview_guide" ? "Interview Guide" : a.pageType === "framework" ? "Framework" : "Guide"}
                    </span>
                    <h3 className="font-black text-sm leading-snug group-hover:text-white transition-colors line-clamp-2">{a.title}</h3>
                    <p className="text-xs text-[var(--text-secondary)] leading-relaxed line-clamp-2">{a.description}</p>
                    <span className="text-[10px] text-[var(--text-secondary)] mt-auto pt-1">{readTime} min read</span>
                  </Link>
                );
              })}
            </div>
      ...
      {/* ── ANSWER ENGINE: COMMON PM QUESTIONS ── */}
      <section className="max-w-5xl mx-auto px-4 sm:px-5 py-12 sm:py-14">
        <div className="mb-8">
          <p className="text-xs font-black uppercase tracking-widest text-[var(--text-secondary)] mb-2">Quick answers</p>
          <h2 className="text-2xl sm:text-3xl font-black leading-tight tracking-tight">Common PM questions</h2>
        </div>
        <div className="grid sm:grid-cols-2 gap-3">
          {[
            { q: "How do I prepare for a PM interview?", a: "Focus on behavioral questions, product sense frameworks (CIRCLES, HEART), and estimation practice. PM Streak's interview guides cover all question types with worked examples.", href: "/learn" },
            { q: "What is a north star metric?", a: "A single metric capturing the core value your product delivers — e.g. Spotify measures minutes listened, Airbnb measures nights booked. Aligning your team around one metric prevents scattered priorities.", href: "/learn" },
            { q: "How do PMs prioritize features?", a: "Top PMs use RICE (Reach × Impact × Confidence ÷ Effort), ICE scoring, or Kano model analysis. Shreyas Doshi recommends separating health metrics from growth metrics before prioritizing anything.", href: "/learn" },
            { q: "What does a product manager do day to day?", a: "PMs define the 'what' and 'why' — writing PRDs, running discovery, aligning stakeholders, reviewing metrics, and unblocking engineering. The ratio of strategy vs. execution work shifts with seniority.", href: "/learn" },
            { q: "What is a product roadmap?", a: "A roadmap communicates the plan for what the team is building and why. Good roadmaps are outcome-oriented (metrics to move) not feature-lists. They're updated quarterly as new data arrives.", href: "/learn" },
            { q: "How do I become a product manager?", a: "Most PMs enter via APM programs (Google, Meta, LinkedIn), internal transitions from engineering/design, or MBA recruiting. Building a portfolio of shipped products and strong data/SQL skills accelerates the path.", href: "/learn" },
          ].map(({ q, a, href }) => (
            <Link
              key={q}
              href={href}
              className="group p-5 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl hover:border-white/15 transition-all"
            >
              <h3 className="font-black text-sm mb-2 group-hover:text-[var(--green-primary)] transition-colors">{q}</h3>
      ...
      <footer className="border-t border-[var(--border-color)] py-6">
        <div className="max-w-5xl mx-auto px-4 sm:px-5 flex flex-wrap items-center justify-between gap-3 text-xs text-[var(--text-secondary)]">
          ...
          <div className="flex items-center gap-4">
            <Link href="/learn" className="hover:text-white transition-colors">PM Guides</Link>
